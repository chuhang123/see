...
class AI_QuickLinks_Widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'ai_quicklinks',
			esc_html__( 'AI - Quick Links', 'extra' ),
			array(
				'description' => esc_html__( 'Quick Links On Sidebar', 'extra' ),
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		<div class="widget_content">
			<ul class="widget_list">
				<li>
					<div style="float:left;"><a class="et-extra-icon et-extra-icon-facebook et-extra-icon-background social-icon" href="http://www.hebut.edu.cn"></a></div>
					<a href="http://www.hebut.edu.cn" class="widget_list_social">
						<h3 class="title">河北工业大学</h3>
						<div class="post-meta">http://www.hebut.edu.cn</div>
	...
	public function form( $instance ) {
		$title = isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : esc_html__( 'Quick Link', 'extra' );
		?>
		<p>
...
class ET_ImageOnly_Widget extends WP_Widget {

	function __construct() {
		parent::__construct(
			'et_imageonly',
			esc_html__( 'Extra - ImageOnly', 'extra' ),
			array(
				'description' => esc_html__( 'Display a picture without border.', 'extra' ),
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		?>
		<!-- <div class="widget_content"> -->
		<a href="<?php echo $instance[ 'imglink' ]; ?>">
			<img src="<?php echo $instance[ 'imgurl' ]; ?>" />
		</a>
		<!-- </div> -->
		<?php
		//echo $args['after_widget'];
	...
	public function form( $instance ) {
		$title = isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : esc_html__( 'Title', 'extra' );
		$imgurl = isset( $instance[ 'imgurl' ] ) ? $instance[ 'imgurl' ] : esc_html__( 'Image URL', 'extra' );
		$imglink = isset( $instance[ 'imglink' ] ) ? $instance[ 'imglink' ] : esc_html__( 'Image Link', 'extra' );
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php esc_html_e( 'Title:', 'extra' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
			<label for="<?php echo $this->get_field_id( 'imgurl' ); ?>"><?php esc_html_e( 'Image URL:', 'extra' ); ?></label>
			...
			<label for="<?php echo $this->get_field_id( 'imglink' ); ?>"><?php esc_html_e( 'Image Link:', 'extra' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'imglink' ); ?>" name="<?php echo $this->get_field_name( 'imglink' ); ?>" type="text" value="<?php echo esc_attr( $imglink); ?>" />
		</p>
		<?php
	}
	...
	public function update( $new_instance, $old_instance ) {
		$instance = array();
		$instance['title'] = ( ! empty( $new_instance['title'] ) ) ? sanitize_text_field( $new_instance['title'] ) : '';
		$instance['imgurl'] = ( ! empty( $new_instance['imgurl'] ) ) ? sanitize_text_field( $new_instance['imgurl'] ) : '';	
		$instance['imglink'] = ( ! empty( $new_instance['imglink'] ) ) ? sanitize_text_field( $new_instance['imglink'] ) : '';	
...
class ET_Ads_Widget extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		<?php $new_line = !empty( $ad['new_line'] ) ? ' new_line' : ''; ?>
			<div class="<?php echo esc_attr( $new_line ); ?>">
...
function extra_widgets_init() {
	register_widget( 'AI_QuickLinks_Widget' );
	...
	register_widget( 'ET_ImageOnly_Widget' );	
